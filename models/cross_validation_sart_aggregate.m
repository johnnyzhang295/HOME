...
rsp_base_215 = load('C:\Users\BIOPACMan\Documents\Zhang\HOME\data\part215\RSP baseline.csv');
    ...
    tabl = table(t,tl,data(:,1),data(:,2),data(:,3),data(:,4),data(:,5), age_sex(:,1), age_sex(:,2),sart,...
        'VariableNames',{'TrialOrder','Taskload','HR','SDNN','pNN50','RSP_Amp','RSP_Rate','Sex','Age','SART'});

    
    mdl = fitglm(tabl,...
        'SART ~ 1 + TrialOrder*pNN50 + TrialOrder*Sex + Taskload*HR + Taskload*RSP_Rate + Taskload*Sex + HR*Age + SDNN*pNN50 + SDNN*RSP_Rate + SDNN*Age + RSP_Amp*Sex + RSP_Rate*Sex',...,
        'ResponseVar','SART','Intercept',true);
%     mdl = stepwiseglm(tabl,...,
%         'SART ~ 1 + Sex+Age + TrialOrder*HR + Taskload*HR + SDNN*pNN50 +SDNN*Age + RSP_Amp*RSP_Rate +RSP_Amp:Sex + RSP_Rate:Sex',...,
%         'ResponseVar','SART','Intercept', true);
%     mdl1 = stepwiseglm(tabl,...,
%         'SART ~ 1 + Sex*Age * TrialOrder*HR * Taskload* SDNN*pNN50',...,
%         'ResponseVar','SART','Intercept', true,'Criterion','Deviance');
%     mdl2 = stepwiseglm(tabl);
%     mdl3 = stepwiseglm(tabl,...,
%         'SART ~ 1 + Sex+Age + TrialOrder+HR +Taskload+ SDNN+pNN50+RSP_Amp+RSP_Rate',...,
%         'ResponseVar','SART','Intercept', true,'Criterion','Deviance');
%     mdl4 = stepwiseglm(tabl,...,
%         'SART ~ 1 + Age*Sex*HR*SDNN*pNN50*TrialOrder*Taskload*RSP_Amp*RSP_Rate',...,
%         'ResponseVar','SART','Intercept', true);


    figure('units','normalized','outerposition',[0 0 1 1]);
    scatter(sart,mdl.Fitted.Response);
    %ylim([1 10]);
    hold on;
    ...
    p = polyfit(sart,mdl.Fitted.Response,1);
    yfit = polyval(p,sart);
    plot(sart,yfit,'-r','HandleVisibility','off');
    grid on;
    grid minor;
    title_1  = strcat('15 Cohort Fit SART');
...
%     figure;
%     scatter(sart,mdl2.Fitted.Response);
%     figure;
%     scatter(sart,mdl3.Fitted.Response);
%     figure;
%     scatter(sart,mdl.Fitted.Response);
    mdl
