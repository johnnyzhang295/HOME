...
load('normalized data.mat');

scale_and_center_data = horzcat(normalize(data(:,1:7)), data(:,8:end));

frma_18 = 'Workload ~ 1 + Sex+Age + TrialOrder*HR + Taskload*HR + SDNN*pNN50 +SDNN*Age + RspAmp*RspRate +RspAmp:Sex + RspRate:Sex+ID';
mdl = fitglm(scale_and_center_data,frma_18);
...
for i=1:12:180
    subplot(3,5,j); 
    hold on;
    plot(scale_and_center_data.Workload(i:i+11),mdl.Fitted.Response(i:i+11),'o','Color',ColOrd(j,:));
    p = polyfit(scale_and_center_data.Workload(i:i+11),mdl.Fitted.Response(i:i+11),1);
    yfit = polyval(p,scale_and_center_data.Workload(i:i+11));
