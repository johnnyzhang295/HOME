...
mdl = fitglm(data,manual_fitting);

%STEP FOUR: Do leave one SUBJECT leave out cross validation
%Go to line 50+
...
formula="";
topTitle = formula;
topTitle = strcat(formula, sprintf('\n Type 4 SA Psychophysiological Regression Model Performance \nAIC = %4.2f BIC = %4.2f R^{2} = %4.2f adjR^{2} = %4.2f',mdl.ModelCriterion.AIC,mdl.ModelCriterion.BIC,mdl.Rsquared.Ordinary,mdl.Rsquared.Adjusted));

suptitle(topTitle);
fn = 'Type 4 SA Psychophysiological Regression Model Performance';
saveas(gcf,strcat('C:\Users\BIOPACMan\Documents\Zhang\HOME\regression models\type4 models\',fn,'.jpg'));
...
topTitle = sprintf('LOOCV Type C For SA Model 4\nQ^{2} = %4.2f ',overall_q_sq);
